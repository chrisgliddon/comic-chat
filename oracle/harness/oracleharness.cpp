...
#ifndef _MSC_VER
#include "../../native/render.h"   // optional render pass; see the end of main()
#endif
...
#ifndef _MSC_VER
    // Optional render pass, macOS only and strictly AFTER the dump has been written.
    //
    // A post-pass rather than a separate driver on purpose: the geometry drawn is then the
    // exact geometry the 15 corpus goldens pin, produced by this same replay, instead of a
    // second interpretation of inputs.json that could drift from it. And because it runs
    // after the dump and touches no engine state, it cannot move a golden.
    //
    // Off unless COMIC_CHAT_RENDER_DIR is set, so the oracle path is unchanged by default.
    if (const char* renderDir = getenv("COMIC_CHAT_RENDER_DIR")) {
        int n = 0, ok = 0;
        POSITION rp = doc->m_pages.GetHeadPosition();
        while (rp) {
            CPage* page = (CPage*)doc->m_pages.GetNext(rp);
            char rpath[MAX_PATH];
            snprintf(rpath, MAX_PATH, "%s/page%02d.png", renderDir, n++);
            int rw = 0, rh = 0;
            NativeRenderPageSize(page, &rw, &rh);
            if (NativeRenderPageToPNG(page, rpath)) {
                printf("rendered %s (%dx%d px)\n", rpath, rw, rh);
                ok++;
            } else {
                fprintf(stderr, "render FAILED for %s\n", rpath);
            }
        }
        printf("rendered %d of %d page(s)\n", ok, n);
    }
#endif
